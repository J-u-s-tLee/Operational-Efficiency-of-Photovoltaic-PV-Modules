...
def encode_categorical(X_train, X_validation, X_test, categorical_cols):
    ...
    return X_train_categorical, X_validation_categorical, X_test_categorical
...
def combine_processed_data(X_train, X_validation, X_test, 
                           ...
                           X_train_numerical_standardized, X_validation_numerical_standardized, X_test_numerical_standardized, numerical_cols,
                           X_train_numerical_normalized, X_validation_numerical_normalized, X_test_numerical_normalized):
    ...
    X_train_combined = pd.concat([
        ...
        pd.DataFrame(X_train_numerical_standardized, index=X_train.index, columns=numerical_cols),
        pd.DataFrame(X_train_numerical_normalized, index=X_train.index, columns=[f"{col}_normalized" for col in numerical_cols])
    ], axis=1)
    ...
    X_validation_combined = pd.concat([
        ...
        pd.DataFrame(X_validation_numerical_standardized, index=X_validation.index, columns=numerical_cols),
        pd.DataFrame(X_validation_numerical_normalized, index=X_validation.index, columns=[f"{col}_normalized" for col in numerical_cols])
    ], axis=1)
    
    X_test_combined = pd.concat([
        pd.DataFrame(X_test_categorical, index=X_test.index, columns=categorical_feature_names),
        pd.DataFrame(X_test_numerical_standardized, index=X_test.index, columns=numerical_cols),
        pd.DataFrame(X_test_numerical_normalized, index=X_test.index, columns=[f"{col}_normalized" for col in numerical_cols])
    ], axis=1)
    
    return X_train_combined, X_validation_combined, X_test_combined


def preprocess_data(X_train, X_validation, X_test, categorical_cols, numerical_cols):
    ...
    X_train_standardized, X_validation_standardized, X_test_standardized, standard_scaler = standardize_numerical(X_train, X_validation, X_test, numerical_cols)
    ...
    X_train_processed, X_validation_processed, X_test_processed = combine_processed_data(X_train, X_validation, X_test,X_train_categorical, X_validation_categorical, X_test_categorical, encoder, categorical_cols, X_train_standardized, X_validation_standardized, X_test_standardized, numerical_cols, X_train_normalized, X_validation_normalized, X_test_normalized)
